...
from type_valuev4 import Closure, Type, Value, create_value, get_printable, Obj
...
class Interpreter(InterpreterBase):
    ...
    def __get_func_by_name(self, name, num_params):
        if name not in self.func_name_to_ast:
            ...
            find_per = name.find('.')
            if find_per != -1: # if it's an object
                obj_name = name[0:find_per]
                obj_val = self.env.get(obj_name)
                if obj_val is None:
                    super().error(ErrorType.NAME_ERROR, f"Object {obj_name} not found")
                
                if obj_val.type() !=  Type.OBJ:
                    super().error(ErrorType.TYPE_ERROR, f"Variable {obj_name} not an object")

                m_call = name[find_per+1:]

                while m_call not in obj_val.v.vars:
                    if obj_val.v.proto is None:
                        super().error(ErrorType.NAME_ERROR, f"Field doesn't exist")
                    obj_val = obj_val.v.proto

                closure_val_obj = obj_val.v.get(m_call)
            ...
            if num_formal_params != num_params:
                if find_per != -1:
                    super().error(ErrorType.NAME_ERROR, "Invalid # of args to method call")
    ...
    def __call_mcall(self, call_ast):
        ...
        m_call = call_ast.get('name')

        func_name = obj_name + "." + m_call

        actual_args = call_ast.get("args")
        target_closure = self.__get_func_by_name(func_name, len(actual_args))
        if target_closure == None:
            super().error(ErrorType.NAME_ERROR, f"Function {m_call} not found")
        if target_closure.type != Type.CLOSURE:
            super().error(ErrorType.TYPE_ERROR, f"Function {m_call} is changed to non-function type.")
    ...
    def __prepare_env_with_closed_variables(self, target_closure, temp_env):
        for var_name, value in target_closure.captured_env:
            ...
            # We only do this for things that aren't closures or Objects

            if self.env.get(var_name) is None or (self.env.get(var_name).type() != Type.OBJ and self.env.get(var_name).type() != Type.CLOSURE):
    ...
    def __assign(self, assign_ast):
        ...
        find_per = var_name.find('.')
        if find_per != -1: # if it's an object
            obj_name = var_name[0:find_per]
            obj_val = self.env.get(obj_name)
            if obj_val is None:
                super().error(ErrorType.NAME_ERROR, f"Object {obj_name} not found")
            ...
            obj_field = var_name[find_per+1:]
            if(obj_field == 'proto'):
                if src_value_obj.t == Type.STRING and src_value_obj.v == "nil":
                    obj_val.v.defined_proto = True
                    obj_val.v.setProto(None)
                elif src_value_obj.t != Type.OBJ and src_value_obj.t != Type.NIL:
                    super().error(ErrorType.TYPE_ERROR, f"Assigning proto to not an object")
                else:
                    obj_val.v.defined_proto = True
                    obj_val.v.setProto(src_value_obj)
            else:
                obj_val.v.set(obj_field, src_value_obj)
        else:
    ...
    def __eval_name(self, name_ast):
        ...
        find_per = var_name.find('.')
        if find_per != -1: # object field 
            obj_name = var_name[0:find_per]
            obj_val = self.env.get(obj_name)
            if obj_val is None: 
                super().error(ErrorType.NAME_ERROR, f"Object {obj_val} not found")
            
            if obj_val.type() !=  Type.OBJ:
                print(obj_val.type())
                super().error(ErrorType.TYPE_ERROR, f"Variable {obj_val} not an object")

            obj_field = var_name[find_per+1:]

            if obj_field == 'proto':
                if not obj_val.v.defined_proto:
                    super().error(ErrorType.NAME_ERROR, f"Field doesn't exist")
                return obj_val.v.proto
            
            while obj_field not in obj_val.v.vars:
                if obj_val.v.proto is None:
                    super().error(ErrorType.NAME_ERROR, f"Field doesn't exist")
                obj_val = obj_val.v.proto

            return obj_val.v.get(obj_field)
        
        if val is not None:
            # if val.type() == Obj:
            #     return Value(Type.OBJ, val)
            return val
